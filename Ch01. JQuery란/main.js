...
-   최근 리액트나 vue.js같은 자바스크립트 프레임워크가 나오면서 트랜드에 밀리고 있지만
    실무에서 여전히 많이 사용한다.
...
/*
라이브러리 적용
- html,css,javascript 이 세가지는 기본적으로 웹페이지에서 라이브러리 없이 인식하는 프론트엔드 언어이다.
- 그 외 자바스크립트 프레임워크들을 사용하려면 라이브러리를 적용해야 한다.
- JQuery를 적용하는 방법은 두 가지가 있다.

1.다운로드 방식
- JQuery 라이브러리 파일을 다운로드 받아서 <script></script> 태그에 연결하는 방식
ex) <script src="JQuery파일경로"></script>
...
/*
준비구문
-<script></script> 태그가 <body></body> 태그보다 위쪽에 써있는 경우 먼저 코드를 읽기 떄문에 문서객체를 읽지
못한 상태에서 실행되어 명령이 적용되지 않는다.
- 준비구문이란 문서 객체를 먼저 읽고 스크립트구문을 실행하라는 구문이다.
- 준비구문은 크게 세가지 종류를 가진다.

1. $(document).ready(function(){});
- 이미지나 동영상이 다 읽혀지지 않고 DOM 만 완성되면 바로 발생하므로 실행속도가 빠르다.
