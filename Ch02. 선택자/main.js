/*
선택자
- JQuery 는 javascript 에서 파생됬기 떄문에 객체선택 방식을 사용한다.
- 객체선택이 쉬워진 것이 특징이다.
- 셀렉터라고도 합니다.
 */
...
:not()          : $('h1:not(.text)') - 앞에 작성된 요소 중 괄호 안의 클래스명이 아닌 것을 선택

:nth-child(n)   : $('div:nth-child(n)') - 해당 요소 중 n번째 작성된 요소를 선택
:first-child    : $('div:first-child') - 해당 요소 중 첫번째로 작성된 요소를 선택
:last-child     : $('div:last-child') - 해당 요소 중 마지막으로 작성죈 요소를 선택
:nth-child(odd) : $('div:nth-child(odd)') - 해당 요소 중 홀수번째 작성된 요소를 선택
:nth-child(even): $('div:nth-child(even)') - 해당 요소 중 짝수번쨰 작성된 요소를 선택

배열요소 (시작이 0 , nth 와 같은 역할)
[***nth-child(1) = eq(0)***]
:eq(n)
:first
:last
...
:text           : $('input:text') - input 태그 중 type 값이 text 인 요소를 선택
:password       : $('input:password') - input 태그 중 type 값이 password 인 요소를 선택
:radio          : $('input:radio') - input 태그 중 type 값이 radio 인 요소를 선택
:checkbox       : $('input:checkbox') - input 태그 중 type 값이 checkbox 인 요소를 선택
:submit         : $('input:submit') - input 태그 중 type 값이 submit 인 요소를 선택
:reset          : $('input:reset') - input 태그 중 type 값이 reset 인 요소를 선택
:button         : $('input:button') - input 태그 중 type 값이 button 인 요소를 선택
:file           : $('input:file') - input 태그 중 type 값이 file 인 요소를 선택
...
$(document).ready(function (){
    ...
    // 0 1 2 3 4 5 odd
    // 1 2 3 4 5 6 nth-child      순서가 달라진다! (홀수를 적용해도 짝수로 인식할 수 있다_)
